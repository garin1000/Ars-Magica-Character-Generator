...
| 15 | **The shipped spell descriptions carry leaked Markdown table syntax.** `rules/i18n/en/spells.json` and its German counterpart were extracted from the rulebook's spell **tables**, and the extractor kept the cell delimiter and the column padding: `spell.the_shadow_of_life_renewed`'s description ends "… raising the dead. \|" and one entry is padded whitespace followed by a bare "\|". Thirteen description lines do not end in sentence punctuation. Unlike the V/F summaries (fixed in `6f06679`) there **is** a generator for this file — `scripts/extract_spells.py` — so the fix belongs there, and the V/F guard in `data_integrity.rs` is deliberately scoped away from spell descriptions until it lands. | re-extraction pass | 2026-09-07 |
