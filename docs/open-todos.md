...
| 13 | **Two Virtues/Flaws still carry a dual-category descriptor joined by *and*/*or* while shipping one category.** `7ea4f5b` resolved three of the original five — Offensive to (Beings) and Unbearable to (Beings) became `["general"]` with real eligibility gates, and Inoffensive to (Beings) gained the gate it was missing. What remains: :5882 `flaw.curse_of_slander` "General or Supernatural" (ships `["general"]`) and :6635 `flaw.primogeniture_lineage` "Story and Hermetic" (`["story"]`). Neither is a wrong-output bug the way the two Flaws were: `general` is on every profile's permitted list and no profile's forbidden list, so Curse of Slander is blocked for nobody; and Primogeniture's real restriction is not a category at all — ":6636" reads "This Flaw can only be taken by magi of House Verditius", which is a House matter, so its single `story` category is probably not the blocking issue either. Adding `hermetic` to Primogeniture would also corrupt Gift detection — see row 18. **Found while narrowing this row, and not previously recorded: that House restriction is unenforced.** `flaw.primogeniture_lineage` ships no `prerequisites` (`rules/core/virtues_flaws.json`), and `story` is permitted to the companion, mythic-companion and magus profiles, so all three can take it today; only the grog is refused, and merely because `story` is not on its permitted list. What the item is missing is a `Prereq::House(house.verditius)` (plus `Prereq::IsMagus`, which the House prereq implies), not a second category. The *or* cases raise a separate question: "General or Supernatural" reads as an either/or origin, not dual membership. | decision | 2026-09-06 |
...
| 20 | **A type profile's category lists are unconditional, and one of them has no source.** (a) ":2840" bars a companion from Hermetic Virtues and Flaws "**unless you have The Gift** (this would be highly unusual)"; the companion profile forbids `hermetic` flatly, so a Gifted companion is refused the exception the book grants. Nothing in the model expresses a conditional category. (b) The grog profile's `forbidden_categories` includes `supernatural` and **`7ea4f5b` could not find a source for it**: ":2824-2830" and ":1009" enumerate a grog's restrictions and neither bars Supernatural Virtues or Flaws, and the `### Supernatural` prose at ":2958-2962" sets no character-type restriction either. It is recorded in RULES.md as an unsourced interpretation rather than a cited rule; either a passage turns up or the entry should go, because it currently blocks builds on the engine's own authority. | decision | 2026-09-09 |
