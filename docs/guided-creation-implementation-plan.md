...
- **i18n keys** en + de: `tab-experience`, `tab-personality_reputations`,
  `tab-aging` (follow the existing `tab-*` naming — check the current keys first).
...
- **Acceptance criteria**
  ...
  - Tab gating matches character type: no empty Experience tab for a type that cannot
    use life-stage funding.
