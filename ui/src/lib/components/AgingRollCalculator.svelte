<script lang="ts">
  ...
  import { displayName, formatSigned, paramHint, resolveIssueArgs } from '../derive';
  import { CHARACTERISTICS } from '../types';
  import type { AgingNote, AgingPointAward, CrisisAllowance, CrisisModifier } from '../ipc';
  ...
  // --- the Crisis (Core Rules.md:16619-16638) -------------------------------
  //
  // "CRISIS TOTAL: Simple die + age/10 (round up) + Decrepitude Score" (`:16621`),
  // read against the Crisis Table (`:16624-16632`).
  //
  // The second die is the player's exactly as the first is, and the app rolls
  // neither. What it will NEVER do is throw the survival roll or say whether the
  // character lived: it reports the Ease Factor, the Creo Corpus level that
  // resolves the crisis instead, every modifier the character carries and what
  // `:16634` allows a doctor to bring — and then stops.
