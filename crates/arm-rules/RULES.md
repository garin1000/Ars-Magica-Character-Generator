...
| Familiar cord cost | `:10836` | cord scores 0…+5 cost **0 / 5 / 15 / 30 / 50 / 75** points: "a strength of +1 requires 5 points, a score of +2 requires 15 points, a score of +3 requires 30 points, a score of +4 requires 50 points, and a score of +5 (the maximum) requires 75 points" (`:10836`). The curve is a fixed 5-entry rule constant, so it is a Rust `const CORD_COST_TABLE` (precedent: `LOAD_TABLE` for Encumbrance) with this row as its provenance home. `cord_points_spent` reports the **total** across all three cords and **clamps** the index at +5 — the store's setter clamps only at 0, the input has no `max`, and a hand-edited save's `u8` can be 255, so a raw index would panic inside the `derived_totals` command and kill the read-out panel (`cord_points_spent_clamps_a_score_above_the_curve`). `cord_points_within_lab_total` reports "The total cost of the cords you buy cannot exceed the magus's Lab Total" (`:10836`). `derived.rs::cord_points_spent` |
