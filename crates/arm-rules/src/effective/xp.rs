...
//!
//! `MAX_XP_SOLVE_NODES` and the `assert!` inside [`xp_allocation`] are a
//! security fix (audit finding K3): an unbounded flow-solve matrix over a
//! crafted save's selections could force a multi-gigabyte single allocation
//! and abort the process on File → Open. The guard stays inside the solver
//! itself so no caller can bypass it — moved here verbatim, unchanged.
...
/// this is a deliberate safety bound, not a game-rule limit.
pub fn xp_allocation(entity: &Entity, ruleset: &Ruleset) -> XpAllocation {
    ...
    // `types.rs:2460,2470,2499,2504`), and the matrix below is `n * n` `u32`s. With
    // no bound, a crafted save with tens of thousands of entries forces a
    ...
    // headroom above that while keeping the matrix under ~64 MB and the
    // Edmonds-Karp solve well under a second. A panic (rather than silently
    // truncating the input) fails loudly, names the offending counts, and — since
    // this workspace sets no `panic = "abort"` override in `Cargo.toml` — unwinds
    // rather than aborting, so it is catchable at a caller boundary instead of
    // killing the process outright. Checked here, inside the solver itself, so it
    // cannot be bypassed by any of `xp_allocation`'s callers (`export.rs`,
    // `arm-app`'s `ruleset_io.rs`, or `restricted_xp_pools` below), current or
    // future.
    assert!(
